...
SITES = {
    ...
    "purse": {
        ...
        "nws_zones": [],
        "use_marine": False,
        "site_type": "river_tidal",
        "clearing": None,
        "pushing": None,
    },
    "westmoreland": {
        "name": "Westmoreland & Stratford Hall (Fossil Beach)",
        "lat": 38.155, "lon": -76.894,
...
def compute_score(weather, alerts, marine=None, clearing_sector=(200,315), site_type="bay_cliff"):
    h      = weather["hourly"]
    now    = datetime.now()
    ...
    # Site-type scoring parameters
    # River/tidal sites: lower rain thresholds (riverbank clay erodes more easily than sea cliffs)
    # and a baseline accessibility score since tidal exposure alone is worth a visit.
    # Bay/cliff sites: original thresholds tuned for Chesapeake wave+wind+cliff conditions.
    is_river = (site_type == "river_tidal")
    if is_river:
        _rain_tiers    = [(1.5, 3), (0.5, 2), (0.15, 1)]  # eff inches → pts
        _peak_iph_min  = 0.35   # lower burst threshold for riverbank erosion
        _peak_cap      = 5      # cap before peak bonus counts
    else:
        _rain_tiers    = [(2.5, 3), (1.0, 2), (0.4, 1)]
        _peak_iph_min  = 0.50
        _peak_cap      = 3
    ...
    for i, ts in enumerate(h["time"]):
        ...
        if cutoff <= t <= now:
            ...
            # clearing: weight strong winds double (bay sites only)
            if not is_river and s >= 10 and in_sector(d, clearing_sector):
    ...
    # Base score — river sites get 2 accessibility points because a low tide at a
    # tidal river site IS inherently worth a visit (fossil beds exposed regardless of weather).
    # Bay/cliff sites start at 0 and require wave/wind/rain events to score.
    if is_river:
        pts = 2
        signals = [{"text": "Tidal river site — low tide exposes fossil beds & eroded banks", "pts": 2}]
    else:
        pts = 0
        signals = []

    # Rainfall score — intensity-weighted + time-decayed accumulation
    # River sites use lower thresholds: any meaningful rainfall erodes clay banks.
    # Bay/cliff sites need higher accumulation to mobilise material at the cliff face.
    rain_label = f"{'Riverbank' if is_river else 'Cliff'} erosion rain past {LOOKBACK_HOURS}h: {rain_past_eff:.2f} eff (raw {rain_past_raw:.2f}\", peak {rain_peak_iph:.2f}\"/hr)"
    for thresh, p_add in _rain_tiers:
        if rain_past_eff >= thresh:
            pts += p_add; signals.append({"text": rain_label, "pts": p_add})
            break

    # Peak intensity bonus: a single heavy burst earns +1 even if total accumulation was low
    if rain_peak_iph >= _peak_iph_min and pts < _peak_cap:
        pts += 1
        signals.append({"text": f"Peak intensity burst: {rain_peak_iph:.2f}\"/hr (high {'bank' if is_river else 'cliff'} erosion event)", "pts": 1})

    # Incoming rain: now scored per-window, not globally

    # NWS advisories — bay/coastal only (river sites don't get coastal/marine alerts)
    if not is_river:
        for a in alerts:
    ...
    if not is_river and clearing_units >= 3:
        pts += 2; signals.append({"text": f"W/SW bay-clearing winds (weighted {clearing_units})", "pts": 2})

    # Marine wave heights (bay/cliff sites only — structured API > NWS text scraping)
    if not is_river:
        if marine:
            try:
                mh = marine["hourly"]
...
def _site_recommendation(site_key, ht, mhw=1.30):
    ...
    if site_key == "calvert":
        ...
        if exposure >= 0.80:
            return "Westmoreland & Stratford Hall \u2014 Fossil Beach fully exposed"
        elif exposure >= 0.60:
            return "Westmoreland & Stratford Hall \u2014 good cliff-base access"
        return "Westmoreland & Stratford Hall \u2014 partial beach (wading shoes helpful)"
...
def build_data():
    ...
    scoring = compute_score(weather, alerts, marine, clearing_sector=SITES["calvert"]["clearing"],
                             site_type=SITES["calvert"]["site_type"])
    ...
    try:
        ...
        score2 = compute_score(w2, [], None, clearing_sector=SITES["purse"]["clearing"],
                               site_type=SITES["purse"]["site_type"])  # alerts omitted for now
    ...
    # Westmoreland & Stratford Hall (Fossil Beach)
    ...
    try:
        ...
        scw = compute_score(ww, [], None, clearing_sector=SITES["westmoreland"]["clearing"],
                           site_type=SITES["westmoreland"]["site_type"])  # river
    ...
    try:
        ...
        scwd = compute_score(wwd, [], None, clearing_sector=SITES["widewater"]["clearing"],
                            site_type=SITES["widewater"]["site_type"])  # river
    ...
    try:
        ...
        scaq = compute_score(waq, [], None, clearing_sector=SITES["aquia"]["clearing"],
                           site_type=SITES["aquia"]["site_type"])
    ...
    return {
        **scoring,
        ...
        "health": {
            k: {
                "ok": v.get("ok"),
                "age_sec": round(time.time() - v["at"], 1) if v.get("at") else None,
                "since_ok_sec": round(time.time() - v["last_ok"], 1) if v.get("last_ok") else None,
                "err": v.get("err"),
            } for k, v in _state.items() if "ok" in v
        }
    }
...
def api_data():
    with _lock:
        cached = _cache["data"]
    if cached is None:
        # Cold-start: background thread hasn't populated cache yet — return loading state
        # rather than blocking synchronously on slow/unavailable upstream APIs.
...
def _refresh_loop():
    ...
    while True:
        try:
            ...
            # NOTE: do not add to _state here — _state only holds known source keys
            # that all have the {ok, err, at, last_ok, data} shape expected by build_data()
...
if __name__ == "__main__":
    ...
    if APPLICATION_ROOT and APPLICATION_ROOT != '/':
        ...
        _root = _Flask('root')
        # Redirect bare root to the fossil dashboard
        @_root.route('/')
        def _root_redirect():
            return redirect(APPLICATION_ROOT, code=302)
